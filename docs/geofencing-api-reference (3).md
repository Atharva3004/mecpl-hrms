...
|------|------------|---------|--------|
| 403 | GEOFENCE_NOT_ENABLED | Geofence attendance is not enabled for your account | Employee's `geofence_attendance` flag is not 'Yes'. Don't show punch UI. |
| 409 | SESSION_ALREADY_OPEN | You already have an open session | Show existing `session_id` and `started_at` from response `data` |
| 409 | ALREADY_PUNCHED_TODAY | You have already completed today's punch-in and punch-out | Hide punch-in button until next calendar day (IST). Response `data` includes `session_id`, `started_at`, `ended_at`, `duration_seconds` of today's completed session. |
...
- Only ONE punch-in/out cycle per calendar day (IST) is allowed. After punch-out, the next punch-in is blocked until the next calendar day.
...
- After punch-out, the next punch-in is blocked until the next calendar day (IST). One punch-in/out cycle per day.
...
[Punch Out] --> Stop location ping timer
    ...
    |
    v
(Punch-In button reappears next calendar day — IST)
```
